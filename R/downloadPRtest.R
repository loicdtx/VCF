...
downloadPR <- function(pr, year, dir, log=NULL,) {
    ...
    if((year == 2000|year == 2005)) {
        ...
        baseURL <- 'ftp://ftp.glcf.umd.edu/glcf/LandsatTreecover/WRS2/'
        year1 <- 20002005
        baseURL1 <- 'ftp://ftp.glcf.umd.edu/glcf/LandsatFCC/WRS2/'
    } else {
    ...
    dl <- function(x, y) { # y is year ; x is the pr element and has already been converted to character
        ...
        if (year == 2000| year == 2005) {
            ...
            url <- sprintf('%s%s%s', baseURL, urlP, urlF)
        } else if (year1 == 20002005) {
            urlP1 <- sprintf('p%s/r%s/p%sr%s_FCC_%d/', p, r, p, r, y) #Path part of the url
            urlF1 <- sprintf('p%sr%s_FCC_%d_CM.tif.gz', p, r, y) # Filename part of the url
            url1 <- sprintf('%s%s%s', baseURL1, urlP1, urlF1)
        } else {
            print("year errors")
        }
        ...
        if (year == 2000) {
            ...
            filename1 <- sprintf('%s/%s%s', dir, urlP1, urlF1)
        } else {
            filename <- sprintf('%s/%s%s', dir, urlP, urlF)
        }
        ...
        if (year == 2000){
            ...
            dir.create(dirname(filename1), showWarnings = FALSE, recursive=TRUE)
        } else {
            dir.create(dirname(filename), showWarnings = FALSE, recursive=TRUE)
        }
        ...
        if (year == 2000) {
            ...
            if (!file.exists(filename1)) {
                print(sprintf('Downloading %s', url1))
                a <- download.file(url=url1, destfile=filename1)
                if (a == 0) {
                    out <- sprintf('%s downloaded successfully', url1)
                } else {
                    out <- sprintf('%s could not be downloaded', url1)
                }      
            } else {
                out <- print(sprintf('File %s already exists, it won\'t be downloaded', basename(filename1)))
            }
        } else {
            if (!file.exists(filename)) {
                print(sprintf('Downloading %s', url))
